	...
	$(function(){
		...
		$("#selected").click(function(){
			...
			costIds.each(function(){
				...
				tab1.append("<tr><td>"+td[1].innerHTML+"<input type='hidden' name='costIds' value='"+td[1].innerHTML+"' ></td>"+
...
<body>
	...
	<form action="expense/updateExpense" id="f1" method="post"
		class="form-horizontal">
		<input type="hidden" name="expenseId" id="expenseId" value="${expenseInfo.expenseId }"/>
				...
				<div class="form-group ">
					...
					<div class="col-xs-9 ">
						<input type="text" class="form-control" required name="expenseName"
							value="${expenseInfo.expenseName }" placeholder="请输入报销原因" />
					</div>
				</div>
			</div>
			<div class="col-xs-7">
				<div class="form-group ">
					...
					<div class="col-xs-5 ">
						<input type="text" class="form-control" readonly="readonly" id="expenseMoney" name="expenseMoney"
							value="${expenseInfo.expenseMoney==null?'0':expenseInfo.expenseMoney }" placeholder="请输入报销总金额" />
					</div>
				</div>
			</div>
			<div class="col-xs-7">
				<div class="form-group ">
					...
					<div class="col-xs-9 ">
						<textarea rows="10" class="form-control" name="expenseDesc" cols=""
							placeholder="请输入报销详情">${expenseInfo.expenseDesc }</textarea>
					</div>
				</div>
			</div>
		</div>
		...
		<table class="table" id="tab1" >
			...
			<c:forEach items="${detailList }" var="di">
				<tr>
					...
					<td>${di.costId }
						<input type="hidden" name="costIds" value="${di.costId }"/>
					</td>
					<td>${di.costName }</td>
					<td>
					<input type='text' value='${di.detailMoney }' id='money'  name='detailMoneys' class='form-control' style='width:100px'>
					</td>
					<td>
					<input type='text' name='detailDescs' value="${di.detailDesc }" class='form-control'>
					</td>
					<td><a href='javascript:void(0)' class='btn btn-danger' onclick='del(this)' >删除</a></td>
				</tr>
